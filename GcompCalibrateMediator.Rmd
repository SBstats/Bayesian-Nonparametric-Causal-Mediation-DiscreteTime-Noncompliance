---
title: "G-computation: Calibrate Mediator (All Treatment Regimes)"
...
# Recent_Buyers_At_Entry_ID = unique(digCom_df$recipient_id[digCom_df$Recent==1 &
#                                                             digCom_df$t ==1])
...
# # Extract data for only 1000 randomly sampled subjects
# first_1500_ID_digCom = sample(digCom_workdf_wide$ID, size = 1500, replace = FALSE)
...
# Do this if 1 chain
#posterior_samples_long = as.data.frame(parameter_samples_edp_long_saved)
...
# posterior_samples_long$`pr_Bin_L0i[1, 10]`
...
sample_Dt = function(MCdata,                             # MC data upto this point in the temporal order
  ...
  for(k in 1:num_D_covs){
   ...
   #print(theta_Dt_colnames)
   #df with nrow = N.M, ncol = num_D_covs
...
#   density = ifelse(x %in% c(0, 1), hurdle_prob,
#                    dtruncnorm(x = x,a = 1, b=Inf, mean =mean, sd = sd))
...
#                                      rbinom(10000,1,0.6)),
#                      MCdensity = check_sample_Dt$density_df,
...
#                      it = 55)
  ...
  random_samples =  rep(fixed_mediator_value, num_MC_samples)
...
MCInteg <- function(var.type, # Vector of variable specifications for data.  Fi=fixed (e.g. the exposure), D=trt received, M1= mediator 1,  M2= mediator 2, Y=outcome. 
  ...
  if (fixed_mediator ==TRUE && is.na(fixed_mediator_value)) stop("You have chosen to fix the mediator values in G-comp but have not provided the value of the fixed mediator")
...
summarize_causal_effect_estimates_across_mediator_values <- function(vec1, mat1, mat2) {
  # Check dimensions
  if (length(vec1) != nrow(mat1) || nrow(mat1) != nrow(mat2)) {
    stop("vec1 must have the same length as the number of rows in mat1 and mat2")
  }

  diffMat <- mat1 - mat2

  # Compute summary statistics
  means <- rowMeans(diffMat)
  lower_CI <- apply(diffMat, 1, quantile, probs = 0.025)
  upper_CI <- apply(diffMat, 1, quantile, probs = 0.975)
  #row_prob_neg <- rowMeans(diffMat < 0)  # proportion of negative samples per row
  ...
  result <- data.frame(
    ...
    #Post_prop_of_neg_effects = row_prob_neg
  )
...
plot_posterior_summary <- function(df, plot_title = "") {

  
  ggplot(df, aes(x = factor(Mediator_value), y = Posterior_means)) +
    geom_point(size = 3, color = "blue") +
    geom_errorbar(aes(ymin = Lower_95_CI, ymax = Upper_95_CI),
                  width = 0.2, color = "darkred") +
    geom_hline(yintercept = 0, linetype = "dashed", color = "gray40") +
    labs(
      title = paste(plot_title, "- posterior means with 95% credible intervals"),
      x = "Mediator value",
      y = "Posterior mean"
    ) +
...
library(patchwork)
plot_posterior_summary_grid <- function(df, plot_title = "", y_limits = NULL) {
  p <- ggplot(df, aes(x = factor(Mediator_value), y = Posterior_means)) +
    geom_point(size = 3, color = "blue") +
    geom_errorbar(aes(ymin = Lower_95_CI, ymax = Upper_95_CI),
                  width = 0.2, color = "darkred") +
    geom_hline(yintercept = 0, linetype = "dashed", color = "gray40") +
  ...
  # Apply uniform y-axis limits if provided
  if (!is.null(y_limits)) {
    p <- p + ylim(y_limits)
  }
...
plot_posterior_summary_band_grid <- function(df, plot_title = "", y_limits = NULL) {
  # Ensure mediator values are ordered factors
  df$Mediator_value <- factor(df$Mediator_value, levels = unique(df$Mediator_value))
  
  p <- ggplot(df, aes(x = Mediator_value, y = Posterior_means, group = 1)) +
    # Shaded credible interval band
    geom_ribbon(aes(ymin = Lower_95_CI, ymax = Upper_95_CI),
                fill = "gray", alpha = 0.3, group = 1) +
    # Line for posterior means
...
## G-computation: Calibrate Mediator — Regime ZVVV (ZstarVVV)

```{r}

input_varType1 = c("L0","L0","L0","Fi", "Dt", "M1t","M2t", "Fi", "Dt", "M1t","M2t","Fi", "Dt", "M1t","M2t", "Y")

fixed_mediator_value_zstar = 0

fixed_mediator_vector_z = c(1, 8, 16, 24)
...
# active_customer_theta_zzstar is a matrix of dim: 1 by 1000
active_customer_theta_zzstar <- MCInteg(var.type = input_varType1,
                                    ...
                                    num_Cont_L0 = 0,
                                    fixed_mediator = TRUE,
                                    fixed_mediator_value = fixed_mediator_value_zstar)

# active_customer_theta_zzstar is a matrix of dim: 4 by 1000
active_customer_theta_zzstar = matrix(rep(active_customer_theta_zzstar, each = 4), nrow = 4)


active_customer_theta_zz <- do.call(rbind, lapply(fixed_mediator_vector_z, function(val) {
                                MCInteg(var.type = input_varType1,
                                  BNPModels = posterior_samples_long,
                                  fixed.regime.trt = c(1,1,1),
                                  fixed.regime.control = c(1,1,1),
                                  U1 = "a",
...
active_customer_theta_zstarzstar = matrix(rep(active_customer_theta_zstarzstar, each = 4), nrow = 4)
...
```{r}
active_customer_PIDE_df = summarize_causal_effect_estimates_across_mediator_values(fixed_mediator_vector_z,
                                                                                active_customer_theta_zzstar,
                                                                            active_customer_theta_zstarzstar)
active_customer_PIDE_df
plot_posterior_summary(active_customer_PIDE_df, "Direct effects")
```
...
```{r}
active_customer_PJIIE_df = summarize_causal_effect_estimates_across_mediator_values(fixed_mediator_vector_z,
                                                                                active_customer_theta_zz,
                                                                            active_customer_theta_zzstar)
active_customer_PJIIE_df
plot_posterior_summary(active_customer_PJIIE_df, "Indirect effects")
```
...
```{r}
active_customer_PCE_df = summarize_causal_effect_estimates_across_mediator_values(fixed_mediator_vector_z,
                                                                                active_customer_theta_zz,
                                                                            active_customer_theta_zstarzstar)
active_customer_PCE_df
plot_posterior_summary(active_customer_PCE_df, "Total effects")
```


### Plot Active Customers effects as a grid

```{r}
# Find global y-axis range across all plots
post_quantiles_AT <- c(active_customer_PIDE_df$Lower_95_CI, active_customer_PIDE_df$Upper_95_CI,
              active_customer_PJIIE_df$Lower_95_CI, active_customer_PJIIE_df$Upper_95_CI,
              active_customer_PCE_df$Lower_95_CI, active_customer_PCE_df$Upper_95_CI)

y_limits_AT <- range(post_quantiles_AT)

# Rebuild with common y-axis
AT_direct_plot <- plot_posterior_summary_grid(active_customer_PIDE_df, "Direct Effect", y_limits_AT)
AT_indirect_plot <- plot_posterior_summary_grid(active_customer_PJIIE_df, "Indirect Effect", y_limits_AT)
AT_total_plot <- plot_posterior_summary_grid(active_customer_PCE_df, "Total Effect", y_limits_AT)

# Arrange as 1x3 grid
AT_plot_grid <- AT_direct_plot | AT_indirect_plot | AT_total_plot
AT_plot_grid
```
...
```{r}
# Rebuild with common y-axis
AT_direct_plot_band <- plot_posterior_summary_band_grid(active_customer_PIDE_df, "Direct Effect", y_limits_AT)
AT_indirect_plot_band <- plot_posterior_summary_band_grid(active_customer_PJIIE_df, "Indirect Effect", y_limits_AT)
AT_total_plot_band <- plot_posterior_summary_band_grid(active_customer_PCE_df, "Total Effect", y_limits_AT)

# Arrange as 1x3 grid
AT_plot_grid_band <- AT_direct_plot_band | AT_indirect_plot_band | AT_total_plot_band
AT_plot_grid_band
```
...
# value_attentive_theta_zzstar is a matrix of dim: 1 by 1000
...
# value_attentive_theta_zzstar is a matrix of dim: 4 by 1000
value_attentive_theta_zzstar = matrix(rep(value_attentive_theta_zzstar, each = 4), nrow = 4)


value_attentive_theta_zz <- do.call(rbind, lapply(fixed_mediator_vector_z, function(val) {
                                MCInteg(var.type = input_varType1,
                                  BNPModels = posterior_samples_long,
                                  fixed.regime.trt = c(1,1,1),
                                  fixed.regime.control = c(1,1,1),
...
value_attentive_theta_zstarzstar = matrix(rep(value_attentive_theta_zstarzstar, each = 4), nrow = 4)
...
```{r}
value_attentive_PIDE_df = summarize_causal_effect_estimates_across_mediator_values(fixed_mediator_vector_z,
                                                                                value_attentive_theta_zzstar,
                                                                            value_attentive_theta_zstarzstar)
value_attentive_PIDE_df
plot_posterior_summary(value_attentive_PIDE_df, "Direct effects")
```
...
```{r}
value_attentive_PJIIE_df = summarize_causal_effect_estimates_across_mediator_values(fixed_mediator_vector_z,
                                                                                value_attentive_theta_zz,
                                                                            value_attentive_theta_zzstar)
value_attentive_PJIIE_df
plot_posterior_summary(value_attentive_PJIIE_df, "Indirect effects")
```
...
```{r}
value_attentive_PCE_df = summarize_causal_effect_estimates_across_mediator_values(fixed_mediator_vector_z,
                                                                                value_attentive_theta_zz,
                                                                            value_attentive_theta_zstarzstar)
value_attentive_PCE_df
plot_posterior_summary(value_attentive_PCE_df, "Total effects")
```


### Plot Value Attentive effects as a grid

```{r}
# Find global y-axis range across all plots
post_quantiles_C <- c(value_attentive_PIDE_df$Lower_95_CI, value_attentive_PIDE_df$Upper_95_CI,
              value_attentive_PJIIE_df$Lower_95_CI, value_attentive_PJIIE_df$Upper_95_CI,
              value_attentive_PCE_df$Lower_95_CI, value_attentive_PCE_df$Upper_95_CI)

y_limits_C <- range(post_quantiles_C)

# Rebuild with common y-axis
C_direct_plot <- plot_posterior_summary_grid(value_attentive_PIDE_df, "Direct Effect", y_limits_C)
C_indirect_plot <- plot_posterior_summary_grid(value_attentive_PJIIE_df, "Indirect Effect", y_limits_C)
C_total_plot <- plot_posterior_summary_grid(value_attentive_PCE_df, "Total Effect", y_limits_C)

# Arrange as 1x3 grid
C_plot_grid <- C_direct_plot | C_indirect_plot | C_total_plot
C_plot_grid
```
...
```{r}
# Rebuild with common y-axis
C_direct_plot_band <- plot_posterior_summary_band_grid(value_attentive_PIDE_df, "Direct Effect", y_limits_C)
C_indirect_plot_band <- plot_posterior_summary_band_grid(value_attentive_PJIIE_df, "Indirect Effect", y_limits_C)
C_total_plot_band <- plot_posterior_summary_band_grid(value_attentive_PCE_df, "Total Effect", y_limits_C)

# Arrange as 1x3 grid
C_plot_grid_band <- C_direct_plot_band | C_indirect_plot_band | C_total_plot_band
C_plot_grid_band
```
...
# price_attentive_theta_zzstar is a matrix of dim: 1 by 1000
...
# price_attentive_theta_zzstar is a matrix of dim: 4 by 1000
price_attentive_theta_zzstar = matrix(rep(price_attentive_theta_zzstar, each = 4), nrow = 4)


price_attentive_theta_zz <- do.call(rbind, lapply(fixed_mediator_vector_z, function(val) {
                                MCInteg(var.type = input_varType1,
                                  BNPModels = posterior_samples_long,
                                  fixed.regime.trt = c(1,1,1),
                                  fixed.regime.control = c(1,1,1),
...
price_attentive_theta_zstarzstar = matrix(rep(price_attentive_theta_zstarzstar, each = 4), nrow = 4)
...
```{r}
price_attentive_PIDE_df = summarize_causal_effect_estimates_across_mediator_values(fixed_mediator_vector_z,
                                                                                price_attentive_theta_zzstar,
                                                                            price_attentive_theta_zstarzstar)
price_attentive_PIDE_df
plot_posterior_summary(price_attentive_PIDE_df, "Direct effects")
```
...
```{r}
price_attentive_PJIIE_df = summarize_causal_effect_estimates_across_mediator_values(fixed_mediator_vector_z,
                                                                                price_attentive_theta_zz,
                                                                            price_attentive_theta_zzstar)
price_attentive_PJIIE_df
plot_posterior_summary(price_attentive_PJIIE_df, "Indirect effects")
```
...
```{r}
price_attentive_PCE_df = summarize_causal_effect_estimates_across_mediator_values(fixed_mediator_vector_z,
                                                                                price_attentive_theta_zz,
                                                                            price_attentive_theta_zstarzstar)
price_attentive_PCE_df
plot_posterior_summary(price_attentive_PCE_df, "Total effects")
```


### Plot Price Attentive effects as a grid

```{r}
# Find global y-axis range across all plots
post_quantiles_D <- c(price_attentive_PIDE_df$Lower_95_CI, price_attentive_PIDE_df$Upper_95_CI,
              price_attentive_PJIIE_df$Lower_95_CI, price_attentive_PJIIE_df$Upper_95_CI,
              price_attentive_PCE_df$Lower_95_CI, price_attentive_PCE_df$Upper_95_CI)

y_limits_D <- range(post_quantiles_D)

# Rebuild with common y-axis
D_direct_plot <- plot_posterior_summary_grid(price_attentive_PIDE_df, "Direct Effect", y_limits_D)
D_indirect_plot <- plot_posterior_summary_grid(price_attentive_PJIIE_df, "Indirect Effect", y_limits_D)
D_total_plot <- plot_posterior_summary_grid(price_attentive_PCE_df, "Total Effect", y_limits_D)

# Arrange as 1x3 grid
D_plot_grid <- D_direct_plot | D_indirect_plot | D_total_plot
D_plot_grid
```
...
```{r}
# Rebuild with common y-axis
D_direct_plot_band <- plot_posterior_summary_band_grid(price_attentive_PIDE_df, "Direct Effect", y_limits_D)
D_indirect_plot_band <- plot_posterior_summary_band_grid(price_attentive_PJIIE_df, "Indirect Effect", y_limits_D)
D_total_plot_band <- plot_posterior_summary_band_grid(price_attentive_PCE_df, "Total Effect", y_limits_D)

# Arrange as 1x3 grid
D_plot_grid_band <- D_direct_plot_band | D_indirect_plot_band | D_total_plot_band
D_plot_grid_band
```
...
# non_active_customer_theta_zzstar is a matrix of dim: 1 by 1000
...
# non_active_customer_theta_zzstar is a matrix of dim: 4 by 1000
non_active_customer_theta_zzstar = matrix(rep(non_active_customer_theta_zzstar, each = 4), nrow = 4)


non_active_customer_theta_zz <- do.call(rbind, lapply(fixed_mediator_vector_z, function(val) {
                                MCInteg(var.type = input_varType1,
                                  BNPModels = posterior_samples_long,
                                  fixed.regime.trt = c(1,1,1),
                                  fixed.regime.control = c(1,1,1),
...
non_active_customer_theta_zstarzstar = matrix(rep(non_active_customer_theta_zstarzstar, each = 4), nrow = 4)
...
```{r}
non_active_customer_PIDE_df = summarize_causal_effect_estimates_across_mediator_values(fixed_mediator_vector_z,
                                                                                non_active_customer_theta_zzstar,
                                                                            non_active_customer_theta_zstarzstar)
non_active_customer_PIDE_df
plot_posterior_summary(non_active_customer_PIDE_df, "Direct effects")
```
...
```{r}
non_active_customer_PJIIE_df = summarize_causal_effect_estimates_across_mediator_values(fixed_mediator_vector_z,
                                                                                non_active_customer_theta_zz,
                                                                            non_active_customer_theta_zzstar)
non_active_customer_PJIIE_df
plot_posterior_summary(non_active_customer_PJIIE_df, "Indirect effects")
```
...
```{r}
non_active_customer_PCE_df = summarize_causal_effect_estimates_across_mediator_values(fixed_mediator_vector_z,
                                                                                non_active_customer_theta_zz,
                                                                            non_active_customer_theta_zstarzstar)
non_active_customer_PCE_df
plot_posterior_summary(non_active_customer_PCE_df, "Total effects")
```


### Plot Non-Active Customers effects as a grid

```{r}
# Find global y-axis range across all plots
post_quantiles_NT <- c(non_active_customer_PIDE_df$Lower_95_CI, non_active_customer_PIDE_df$Upper_95_CI,
              non_active_customer_PJIIE_df$Lower_95_CI, non_active_customer_PJIIE_df$Upper_95_CI,
              non_active_customer_PCE_df$Lower_95_CI, non_active_customer_PCE_df$Upper_95_CI)

y_limits_NT <- range(post_quantiles_NT)

# Rebuild with common y-axis
NT_direct_plot <- plot_posterior_summary_grid(non_active_customer_PIDE_df, "Direct Effect", y_limits_NT)
NT_indirect_plot <- plot_posterior_summary_grid(non_active_customer_PJIIE_df, "Indirect Effect", y_limits_NT)
NT_total_plot <- plot_posterior_summary_grid(non_active_customer_PCE_df, "Total Effect", y_limits_NT)

# Arrange as 1x3 grid
NT_plot_grid <- NT_direct_plot | NT_indirect_plot | NT_total_plot
NT_plot_grid
```
...
```{r}
# Rebuild with common y-axis
NT_direct_plot_band <- plot_posterior_summary_band_grid(non_active_customer_PIDE_df, "Direct Effect", y_limits_NT)
NT_indirect_plot_band <- plot_posterior_summary_band_grid(non_active_customer_PJIIE_df, "Indirect Effect", y_limits_NT)
NT_total_plot_band <- plot_posterior_summary_band_grid(non_active_customer_PCE_df, "Total Effect", y_limits_NT)

# Arrange as 1x3 grid
NT_plot_grid_band <- NT_direct_plot_band | NT_indirect_plot_band | NT_total_plot_band
NT_plot_grid_band
```
...
### Plot indirect effect bands across strata

```{r}
# Find global y-axis range across all plots
post_quantiles_ALL_PJIIE <- c(active_customer_PJIIE_df$Lower_95_CI, active_customer_PJIIE_df$Upper_95_CI,
                              value_attentive_PJIIE_df$Lower_95_CI, value_attentive_PJIIE_df$Upper_95_CI,
                              price_attentive_PJIIE_df$Lower_95_CI, price_attentive_PJIIE_df$Upper_95_CI,
                              non_active_customer_PJIIE_df$Lower_95_CI, non_active_customer_PJIIE_df$Upper_95_CI
                            )

y_limits_ALL <- range(post_quantiles_ALL_PJIIE)

# Rebuild with common y-axis
AT_indirect_plot_band <- plot_posterior_summary_band_grid(active_customer_PJIIE_df, "Indirect Effect", y_limits_ALL)
C_indirect_plot_band <- plot_posterior_summary_band_grid(value_attentive_PJIIE_df, "Indirect Effect", y_limits_ALL)
D_indirect_plot_band <- plot_posterior_summary_band_grid(price_attentive_PJIIE_df, "Indirect Effect", y_limits_ALL)
NT_indirect_plot_band <- plot_posterior_summary_band_grid(non_active_customer_PJIIE_df, "Indirect Effect", y_limits_ALL)

# Arrange as 1x4 grid
ALL_indirect_plot_grid_band <- AT_indirect_plot_band | C_indirect_plot_band | D_indirect_plot_band | NT_indirect_plot_band
ALL_indirect_plot_grid_band
```




## G-computation: Calibrate Mediator — Regime ZPPP (ZstarPPP)
...
## G-computation: Calibrate Mediator — Regime ZPVV (ZstarPVV)
...
## G-computation: Calibrate Mediator — Regime ZPPV (ZstarPPV)
...
## G-computation: Calibrate Mediator — Regime ZVPP (ZstarVPP)
...
## G-computation: Calibrate Mediator — Regime ZPVP (ZstarPVP)
...
## G-computation: Calibrate Mediator — Regime ZVPV (ZstarVPV)
...
## G-computation: Calibrate Mediator — Regime ZVVP (ZstarVVP)
