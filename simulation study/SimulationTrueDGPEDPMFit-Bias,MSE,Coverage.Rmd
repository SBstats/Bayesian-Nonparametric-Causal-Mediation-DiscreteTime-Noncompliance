...
head(digCom_df)
##################Load subset data#################################
#digCom_df = get(load("subset_non-comp_dataset_n5000.Rdata"))
...
code_DG <- nimbleCode({
  ...
  ##############################Hurdle probabilities################################
    ...
    # AT t =3
    for(k in 1:num_Y_reg_coeff){beta_Yi[k]~ dnorm(mean = 0, sd = 10)}  
    for(k in 1:num_M2t3_reg_coeff){theta_M2_t3[k]~ dnorm(mean = 0, sd = 10)} 
    for(k in 1:num_M1t3_reg_coeff){theta_M1_t3[k]~ dnorm(mean = 0, sd = 10)} 
    for(k in 1:num_Dt3_reg_coeff){theta_D_t3[k]~ dnorm(mean = 0, sd = 10)} 
    for(k in 1:num_Zt3_reg_coeff){theta_Z_t3[k]~ dnorm(mean = 0, sd = 10)} 
    
    # AT t =2
    for(k in 1:num_M2t2_reg_coeff){theta_M2_t2[k]~ dnorm(mean = 0, sd = 10)} 
    for(k in 1:num_M1t2_reg_coeff){theta_M1_t2[k]~ dnorm(mean = 0, sd = 10)} 
    for(k in 1:num_Dt2_reg_coeff){theta_D_t2[k]~ dnorm(mean = 0, sd = 10)} 
    for(k in 1:num_Zt2_reg_coeff){theta_Z_t2[k]~ dnorm(mean = 0, sd = 10)} 
    
    # AT t =1
    for(k in 1:num_M2t1_reg_coeff){theta_M2_t1[k]~ dnorm(mean = 0, sd = 10)} 
    for(k in 1:num_M1t1_reg_coeff){theta_M1_t1[k]~ dnorm(mean = 0, sd = 10)} 
    for(k in 1:num_Dt1_reg_coeff){theta_D_t1[k]~ dnorm(mean = 0, sd = 10)} 
    for(k in 1:num_Zt1_reg_coeff){theta_Z_t1[k]~ dnorm(mean = 0, sd = 10)} 
    
    
    #####################Regression variance coefficients################################
    sigma_sq_Yi ~ dinvgamma(shape = 1, scale = 1)
    sigma_sq_M2_t3 ~ dinvgamma(shape = 1, scale = 1)
    sigma_sq_M1_t3 ~ dinvgamma(shape = 1, scale = 1)
    sigma_sq_M2_t2 ~ dinvgamma(shape = 1, scale = 1)
    sigma_sq_M1_t2 ~ dinvgamma(shape = 1, scale = 1)
    sigma_sq_M2_t1 ~ dinvgamma(shape = 1, scale = 1)
    sigma_sq_M1_t1 ~ dinvgamma(shape = 1, scale = 1)  
...
# burn-in 8000 iterations and save 1 iter for data generation
